...
export default class HealerBehavior extends NPCBehavior  {
    ...
    public initializeAI(owner: NPCActor, opts: Record<string, any>): void {
        ...
        let lowhealthAlly = new BasicFinder<Battler>(null, BattlerActiveFilter(), BattlerGroupFilter([owner.battleGroup], owner.battleGroup === 2)); // this code base is buggy so I add this argument
        this.addStatus(HealerStatuses.ALLY_EXISTS, new TargetExists(scene.getBattlers(), lowhealthAlly));
        
        /* ######### Add all healer actions ######## */

        // TODO configure the rest of the healer actions -- done

        // An action for healing an ally battler
        let healAlly = new UseHealthpack(this, this.owner);
        healAlly.targets = scene.getBattlers();
        healAlly.targetFinder = new BasicFinder<Battler>(ClosestPositioned(this.owner), BattlerActiveFilter(), AllyFilter(this.owner), BattlerHealthFilter(0, 6));
